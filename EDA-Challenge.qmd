...
The above plotted values seems to be skewed to right. Therefore, we can use log values of the variables for the plot. If we convert the values to log values, the minus(-) values for day range length variable will indicate the values between 0 and 1.
...
d<-d[ is.finite(log(d$Move)) & is.finite(log(d$DayLength_km)), ] # Removing invalid values after log convrersion
p<-ggplot(d, aes(x = log(Move), y = log(DayLength_km)))
p<-p+ geom_point(alpha=0.5,aes(color=Family),size=2, na.rm = TRUE)
p<-p+ geom_smooth(method = lm, se = FALSE)
...
-   Above overall conclusion varies slightly if we consider families separately. Cercopithecidae and Atelidae show a considerable good positive relationship, from which we can tell that the species spend more time moving generally travel farther within a day.
...
-   Lemuridae shows a flat relationship while only Hominidae shows a negative relationship which tells us that the time they spent moving is less allocated to move farther away or the time they spent moving is not necessarily for travelling farther away respectively.
